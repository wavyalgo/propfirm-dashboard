    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#050911] text-slate-800 dark:text-slate-200 font-sans selection:bg-emerald-500/30 overflow-x-hidden relative transition-colors duration-300">
            ...
            <div className="max-w-7xl mx-auto p-6 lg:p-10 space-y-12 relative z-10">
                    ...
                    <Link to="/" className="group cursor-pointer">
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                                Propfirm <span className="text-slate-500 dark:text-slate-500 group-hover:text-slate-400 transition-colors">資金分析</span>
                            </h1>
                        </div>
                        <p className="text-slate-600 dark:text-slate-400 text-sm font-medium ml-1 group-hover:text-slate-500 dark:group-hover:text-slate-300 transition-colors">追蹤您的交易資金流向</p>
                    </Link>
                ...
                <div className="bg-white dark:bg-[#111827] p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-lg flex flex-col md:flex-row items-center gap-2 animate-in fade-in slide-in-from-top-4 duration-500">
                    ...
                    <div className="hidden md:block w-px h-8 bg-slate-300 dark:bg-slate-700 mx-2"></div>

                    {/* Date Filter */}
                    <div className="flex items-center gap-2 bg-slate-50 dark:bg-black/20 p-1 rounded-xl border border-slate-300 dark:border-slate-700">
                        <input
                            type="date"
                    ...
                    <div className="flex-1 w-full flex flex-wrap items-center gap-2 px-1">
                        <div className="text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-wider mr-1 hidden md:flex items-center gap-1">
                            <span className="text-[10px] bg-slate-200 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-500">FILTER</span>
                        </div>
                        {filterOptions.map(firmName => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${selectedFirms.includes(firmName)
                                    ? 'bg-emerald-500 text-white border-emerald-500 shadow-md shadow-emerald-500/20' // Active
                                    : 'bg-transparent text-slate-600 dark:text-slate-400 border-transparent hover:bg-slate-200 dark:hover:bg-slate-800/50 hover:text-emerald-600 dark:hover:text-emerald-400' // Inactive
                                    }`}
                            >
                                {firmName}
                        ...
                        {selectedFirms.length > 0 && (
                            <button
                                ...
                                className="ml-auto text-xs font-bold text-rose-400 hover:text-rose-300 bg-rose-500/10 hover:bg-rose-500/20 px-3 py-1.5 rounded-lg transition-colors"
                            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <StatCard title="總投資成本" value={financeStats.totalExpenses} prefix="$" icon={<ArrowDownCircle />} color="text-rose-400" trend="-2.5%" trendUp={false} />
                    <StatCard title="總出金收益" value={financeStats.totalRevenue} prefix="$" icon={<ArrowUpCircle />} color="text-emerald-400" trend="+12.4%" trendUp={true} />
                    <StatCard title="淨利潤" value={financeStats.netProfit} prefix="$" icon={<DollarSign />} color={financeStats.netProfit >= 0 ? "text-cyan-400" : "text-rose-400"} trend="Active" trendUp={true} />
                    <StatCard title="投資回報率 (ROI)" value={financeStats.roi} suffix="%" icon={<Activity />} color="text-purple-400" trend="All time" trendUp={true} />
                </div>
